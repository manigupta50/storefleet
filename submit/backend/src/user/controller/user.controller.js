...
export const forgetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ success: false, msg: 'No user found with provided email' });
    }
    const resetPasswordToken = await user.getResetPasswordToken();
    sendPasswordResetEmail(req.body, resetPasswordToken);
  } catch (error) {
    console.error(error);
  }
};
...
export const resetUserPassword = async (req, res, next) => {
  ...
  try {
    ...
    const user = findUserForPasswordResetRepo(hashedToken);
    ...
    await updatePasswordRepo(user._id, hashedPassword);
...
export const updatePassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!newPassword || newPassword !== confirmPassword) {
      return next(new ErrorHandler(401, 'mismatch new password and confirm password!'));
    }
...
export const updateUserProfileAndRole = async (req, res, next) => {
  ...
  try {
    const userUpdated = await updateUserRoleAndProfileRepo(req.params.id, req.body);
    if (!userUpdated) {
      return res
        .status(400)
        .json({ success: false, msg: 'Can not update role of this user' });
    }
